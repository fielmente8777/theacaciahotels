import Form1 from "@/components/forms/Form1";
import {
  Container,
  Section,
  SectionWithContainer,
} from "@/components/sectionComponants";
import { SectionHeading } from "@/components/typography";

interface FormSectionProps {
  location: string;
  title: string;
  subtitle: string;
}
const FormSection: React.FC<FormSectionProps> = ({
  location,
  title,
  subtitle,
}) => {
  return (
    <Section>
      <Container className="flex flex-col gap-4 items-center justify-center">
        <p className="flex items-center gap-2">
          <span>
            {" "}
            <Foo />{" "}
          </span>{" "}
          {location}
        </p>
        <SectionHeading title={title} textCenter />
        <p>{subtitle}</p>
      </Container>
      <Container>
        <div
          className="bg-primary p-4 lg:rounded-tl-xl lg:rounded-br-xl mt-6 lg:rounded-bl-4xl max-lg:rounded-xl lg:rounded-tr-4xl"
          id="form"
        >
          <Form1 />
        </div>
      </Container>
    </Section>
  );
};

export default FormSection;

export const Foo = () => (
  <svg
    width={15}
    height={19}
    viewBox="0 0 15 19"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M5.92917 17.3425C6.43417 17.7592 6.96083 18.14 7.5 18.5117C8.04033 18.1449 8.56444 17.7548 9.07083 17.3425C9.915 16.6493 10.7095 15.8978 11.4483 15.0933C13.1517 13.2308 15 10.5308 15 7.5C15 6.51509 14.806 5.53982 14.4291 4.62987C14.0522 3.71993 13.4997 2.89314 12.8033 2.1967C12.1069 1.50026 11.2801 0.947814 10.3701 0.570903C9.46018 0.193993 8.48491 0 7.5 0C6.51509 0 5.53982 0.193993 4.62987 0.570903C3.71993 0.947814 2.89314 1.50026 2.1967 2.1967C1.50026 2.89314 0.947814 3.71993 0.570903 4.62987C0.193993 5.53982 -1.46764e-08 6.51509 0 7.5C0 10.5308 1.84833 13.23 3.55167 15.0933C4.29052 15.8981 5.08497 16.6491 5.92917 17.3425ZM7.5 10.2083C6.78171 10.2083 6.09283 9.92299 5.58492 9.41508C5.07701 8.90717 4.79167 8.21829 4.79167 7.5C4.79167 6.7817 5.07701 6.09283 5.58492 5.58492C6.09283 5.07701 6.78171 4.79167 7.5 4.79167C8.21829 4.79167 8.90717 5.07701 9.41508 5.58492C9.92299 6.09283 10.2083 6.7817 10.2083 7.5C10.2083 8.21829 9.92299 8.90717 9.41508 9.41508C8.90717 9.92299 8.21829 10.2083 7.5 10.2083Z"
      fill="#A3844D"
    />
  </svg>
);
